...
Вы можете настроить ограничения частоты запросов (rate limiting) и белый список разрешенных чатов в вашем файле `.env` для защиты квоты Gemini API:
...
*   **Белый список разрешенных чатов (Chat Authorization Whitelist)**:
    *   `ALLOWED_CHATS`: Список идентификаторов (ID) чатов через запятую, которым разрешено использовать бота (например, `-100123456789,-100987654321,12345678`). Если переменная не задана или пуста, авторизация отключена, и любой чат/пользователь может использовать бота.
